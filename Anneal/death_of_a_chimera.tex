...
% Port to the official AIP/Chaos template at task c10 (preamble swap only).
...
\title{\bfseries Death of a chimera: breath-synchronized collapse of a\\ post-homoclinic transient in a two-population oscillator system}
\author{Avery Karlin\\ \small ak5232@columbia.edu \quad github.com/akarlin3/annealMusic}
\date{}
...
re-forms, and 69--97\% of detector firings are such self-healing \emph{grazes}.
...
corner ($A = 0.2$) holds the system's only true attractor ($r^* = 0.678$; 51--78\%
...
fluctuations \emph{prolong} the transient --- an inversion of the usual
noise-kills-metastability intuition; a direct test rules out additive collective
noise, leaving the mechanism open.
\end{abstract}

\noindent\textit{Chimera states, in which a population of identical oscillators
...
Watanabe--Strogatz theory~\cite{ws1993,ws1994} confines each population to a
three-parameter manifold at \emph{every} $N$ --- as the group-theoretic~\cite{mms2009}
and hierarchical-population~\cite{pr2008} formulations make explicit --- so the
three-variable flow integrated here is the exact collective dynamics, not a
...
figure-generation scripts --- are available in the annealMusic repository at
github.com/akarlin3/annealMusic and will be deposited, with a citable archival DOI,
upon submission; python3 tools/paper-figures/run\_all.py regenerates all figures.
