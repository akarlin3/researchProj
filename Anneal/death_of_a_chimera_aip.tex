...
\textbf{A constrained hypothesis for the prolongation.} The constraints above
admit one concrete hypothesis, which we state strictly as such, to fix a target
for analytic work. In the reduced flow, death is decided in the slow passages
near the saddle, where the spiral's exit from the ghost region is determined. We
conjecture that finite-$N$ fluctuations act there as a fluctuation-induced
drift: integrated over the long residence near the saddle, they systematically
displace the trajectory off the deterministic exit path, returning it to the
spiral for additional whole breath cycles rather than letting it absorb.
Re-injection in integer breath cycles would preserve the breath-locked death
phases and the aging-in-cycles hazard --- the ratchet keeps its clock; only the
cycle count changes --- and a missed-exit probability set by the local saddle
geometry weakens with depth beyond the homoclinic, as observed at $A = 0.5$. The
hard constraint remains the $N$-independence: instantaneous finite-$N$
fluctuations scale as $1/\sqrt N$, so the hypothesis requires the displacement
accumulated per saddle passage to stay $N$-flat --- a compensation between
shrinking amplitude and the growing time a quieter trajectory lingers near the
saddle, of the kind familiar from the logarithmic noise-sensitivity of exit
times near saddles. The target for future work is therefore definite: a
Fokker--Planck (equivalently, backward-Kolmogorov first-passage) treatment of
the three-variable reduced flow, with the state-dependent diffusion inherited
from finite-$N$ sampling, evaluated in the saddle region, must produce a
per-cycle missed-exit probability that is flat in $N$ at the operating corner,
falls with depth, and is consistent with the rising hazard --- or the hypothesis
fails. It also makes one falsifiable prediction: at sufficiently large $N$,
once the sampling amplitude drops below the scale of the exit channel, the
prolongation factor must decay toward one. We advance it as exactly that: a
constrained hypothesis to guide the derivation, not a result.
