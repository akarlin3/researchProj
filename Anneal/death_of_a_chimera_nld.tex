...
holds the only true attractor --- and a reduced-flow sweep shows the inversion is
generic across the post-homoclinic wedge while the aging holds only beyond a
finite depth, going memoryless within ${\approx}0.02$ of the homoclinic. Yet the
reduced flow captures $3.2\times$ faster than the finite-$N$ system dies; direct
tests rule out additive collective noise, breath-locked multiplicative noise, and
the off-manifold Watanabe--Strogatz constants, localizing the prolongation to
finite-$N$ sampling on the Poisson submanifold and leaving its theory open. On
the ring, just inside an independently measured existence boundary
($\beta_c = 0.137$ [$0.130, 0.144$]; $\beta = 0.110$--$0.130$, $N = 32$--$256$,
$7{,}500$ pre-registered runs), collapse is again an aging
