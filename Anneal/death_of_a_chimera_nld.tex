...
\author*[1,2]{\fnm{Avery} \sur{Karlin}\orcid{0000-0003-3848-6782}}\email{ak5232@columbia.edu}

\affil*[1]{\orgdiv{Department of Applied Physics and Applied Mathematics}, \orgname{Columbia University}, \orgaddress{\city{New York}, \state{NY}, \postcode{10027}, \country{USA}}}

\affil[2]{\orgdiv{Department of Computer Science}, \orgname{University of Colorado Boulder}, \orgaddress{\city{Boulder}, \state{CO}, \postcode{80309}, \country{USA}}}

\abstract{Which features of chimera collapse are universal, and which belong to the
coupling topology? We answer with matched long-duration survival measurements on the
two canonical chimera substrates: the two-population Sakaguchi--Kuramoto mean field
with strictly identical natural frequencies ($N = 4$--$64$ per population, the
oscillator engine of a deployed real-time additive synthesizer) and the nonlocally
coupled ring near its existence boundary, which we measure independently at
$\beta_c = 0.137$ [$0.130, 0.144$] ($\beta = 0.110$--$0.130$, $N = 32$--$256$,
$7{,}500$ pre-registered runs). In both systems, collapse ages \emph{at the hazard
level}: the instantaneous death rate rises with the state's lifetime. In the mean
field, censored-Weibull shapes $k = 2.3$--$3.0$ survive stratification on the
collective initial condition, and death arrives by a breath-synchronized ratchet ---
per-cycle envelope maxima climb monotonically to capture. On the ring, $k$ exceeds
one in all 15 pre-registered $(\beta, N)$ cells and saturates to a finite
$k_\infty \approx 1.35$--$1.65 > 1$ that sharpens toward the measured $\beta_c$. The
finite-size scaling, by contrast, is topology-dependent: mean-field absorption
lifetimes are flat ($\tau_{\mathrm{abs}} \approx 139$\,s over a $16\times$ range of
$N$) while near-boundary ring medians \emph{decrease} with $N$. The mean-field half
also corrects a measurement habit: first-passage criteria misfire there, with up to
98\% of criterion-satisfying crossings recovering and 69--97\% of the deployed
detector's firings landing on self-healing near-synchrony \emph{grazes}, so an
absorption-grade label doubles true lifetimes and inverts the stability map --- the
operating corner lies beyond the homoclinic of the parameter-free reduced flow,
validated against its source's published bifurcations and confirmed across the
$(\beta, A)$ plane, with the aging signature onsetting at homoclinic depth. That gap
is specific to the breathing mean field: on the ring first-passage and
absorption-grade labels coincide. The reduced flow underpredicts finite-$N$ lifetimes
by a fixed $3.2\times$; additive collective noise, multiplicative breath-locked
noise, and the off-manifold Watanabe--Strogatz constants are each ruled out by direct
test, leaving the prolongation mechanism a sharply constrained open problem. The
aging belongs to the chimera; the scaling belongs to the network.}
...
finite at feasible compute, asking whether the aging structure of collapse --- a
rising hazard, i.e., an instantaneous death rate that grows with the age of the
state --- transfers across topology even where the lifetime scaling does not.
...
process: the censored-Weibull shape --- the exponent $k$ that separates a
memoryless collapse ($k = 1$: exponential waiting times, an age-independent
hazard) from an aging one ($k > 1$: a death rate that rises as the state grows
older) --- exceeds one, with confidence intervals
...
with right-censoring at $T_{\max} = 12{,}000$ --- a run still alive at the horizon
is not discarded or counted as a death, but enters every likelihood below only
through the fact that it survived to $T_{\max}$; Sec.~\ref{sec:ring} validates this
...
lifetimes computed with engineering-style criteria here measure the wrong,
and roughly halved, quantity --- a gap that
Sec.~\ref{sec:ring:xlabel} shows is a property of these breathing dynamics
rather than of first-passage labeling in general.
...
The natural null for collapse is memoryless escape: a transient leaving a chaotic saddle at a constant per-unit-time rate --- the standard chaotic-transient picture, and the behaviour Ref.~\cite{wo2011} establishes for ring chimeras --- gives exponential lifetimes, i.e.\ Weibull shape $k = 1$. Departure from $k = 1$ is therefore the non-trivial signal. Absorption at $A = 0.5$ is not memoryless in this sense. Censored-Weibull fits give shape
...
age dependence, $k = 1$, exponential waiting times. At the operating corner that
null fails at every size and under every stratification above. It does not fail
everywhere the chimera is post-homoclinic, however: sweeping the reduced flow
across the $(\beta, A)$ plane (Appendix~\ref{app:corner}) shows the aging shape
survives at the operating corner's depth beyond the homoclinic for every $\beta$
tested, but within ${\approx}0.02$ of the homoclinic at $\beta \le 0.10$ the
reduced collapse-time distribution is statistically indistinguishable from the
memoryless null ($\hat k = 0.97$--$1.02$, intervals straddling one). Aging is a
property of the corner's depth beyond the homoclinic, not of post-homoclinicity
per se.
...
The mean-field system entered this study as an instrument, not as a choice of
substrate: a deployed synthesizer, run unattended for durations no deliberate
numerical campaign would have scheduled, is what exposed the metric flaw --- a
collapse detector that fires on grazes the chimera survives
(Sec.~\ref{sec:graze}) --- and every corrected measurement since has been made
on that one instrument. The physics recovered after the correction, an aging
hazard with size-independent lifetimes, is therefore so far a fact about one
coupling topology. The ring enters in a different role: not as where the
phenomenon happened to live, but as the canonical substrate on which the aging
must be re-measured to establish whether it decouples from the all-to-all
topology that hosted its discovery.

\subsection{Why the ring, and where on it}

The mean-field system showed aging without lifetime growth; is either property an
artifact of that topology? The nonlocally coupled ring of
...
Collapse on the ring is not a memoryless escape. As in the mean field, the memoryless null --- a constant escape rate giving Weibull shape $k = 1$~\cite{wo2011} --- is the baseline, and the data reject it. Across all 15 pre-registered
...
Against the same memoryless null as the mean field
(Sec.~\ref{sec:stats}), the ring leaves no measured refuge: the exponential is
rejected in every one of the 15 pre-registered cells, the refitted shape stays
above one under every alternative death criterion tested below, and the null is
not recovered in the thermodynamic limit --- the bootstrapped $k_\infty$
intervals exclude one at every $\beta$ (Appendix~\ref{app:ring}). Whether the
ring possesses an analog of the mean field's near-homoclinic memoryless band
(Appendix~\ref{app:corner}) --- a proximity to the boundary below which the
aging shape fails --- is not resolved by this campaign, whose closest approach
is $\beta = 0.130$ against $\beta_c = 0.137$ [$0.130, 0.144$].
...
lifetime (Spearman $|\rho| \le 0.14$ in every campaign cell, $\le 0.21$
including the $N = 192$--$256$ extension cells) over lifetimes spanning
...
\textbf{A constrained hypothesis for the prolongation.} The nulls above localize
the missing mechanism tightly enough that one concrete hypothesis is worth
stating --- strictly as a hypothesis, to fix the target for analytic work. The
prolongation accrues in the fluctuation-induced recoveries between the first
sustained $\theta$-crossing and absorption, i.e.\ in the slow passages near the
saddle where exit from the ghost region is decided. We conjecture that finite-$N$
sampling of the on-manifold flow acts there as a fluctuation-induced drift:
integrated over the long residence near the saddle, the sampling fluctuations
systematically displace the trajectory off the deterministic exit path, returning
it to the spiral for additional whole breath cycles rather than letting it
absorb. This one mechanism would carry the right signatures: re-injection in
integer breath cycles preserves the breath-locked death phases and the
aging-in-cycles hazard (the ratchet keeps its clock; only the cycle count
changes), and a missed-exit probability set by the local saddle geometry weakens
with depth beyond the homoclinic, as observed at $A = 0.5$. The hard constraint
is the $N$-independence: instantaneous sampling fluctuations scale as
$1/\sqrt N$, so the hypothesis requires the displacement accumulated per saddle
passage to stay $N$-flat --- a compensation between shrinking amplitude and the
growing time a quieter trajectory lingers near the saddle, of the kind familiar
from the logarithmic noise-sensitivity of exit times near saddles. Whether that
compensation is exact enough to hold over a $16\times$ range of $N$, and at what
$N$ it must eventually fail (the hypothesis predicts the factor decays toward one
once the sampling amplitude drops below the scale of the exit channel), is
precisely what an analytic treatment would decide. The target is therefore
definite: a Fokker--Planck (equivalently, backward-Kolmogorov first-passage)
analysis of the three-variable reduced flow, with the state-dependent diffusion
inherited from finite-$N$ sampling on the Poisson submanifold, evaluated in the
saddle region, must produce a per-cycle missed-exit probability that is flat in
$N$ at the operating corner, falls with depth, and is consistent with the rising
hazard --- or the hypothesis fails. We advance it as exactly that: a constrained
hypothesis to guide the derivation, not a result.
...
citation into a measurement (Fig.~\ref{fig:crosssystem}). On the near-boundary side of the ring's existence
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{fig12_crosssystem}
\caption{Cross-system comparison: aging transfers, scaling does not.
(a)~Median lifetime versus size, each system normalized to its smallest
campaign size: the mean-field absorption plateau is flat over a $16\times$
range of $N$ ($\tilde\tau(64)/\tilde\tau(4) = 1.00$), while the ring's
near-boundary median falls $2.5\times$ over an $8\times$ range
($872 \to 345$ time units at $\beta = 0.130$). (b)~Censored-Weibull shape
versus size for the same campaigns: mean-field $k_{\mathrm{abs}} =
2.27$--$3.03$ and ring $\hat k = 1.33$--$1.68$, every interval above the
memoryless $k = 1$ (dashed); the band is the ring's bootstrapped thermodynamic
limit $k_\infty = 1.65$ [$1.56, 1.74$] at $\beta = 0.130$
(Appendix~\ref{app:ring}). (c)~Nelson--Aalen cumulative hazard versus
normalized age $t/\tilde\tau(N)$ (lifetimes median-normalized per cell,
pooled within each system; zero censoring in both campaigns): both curves
are convex against the memoryless reference $H = u \ln 2$ --- the hazard
rises with age in both systems --- with the ring's late bend back toward
slope one marking its front-loaded rise.}
\label{fig:crosssystem}
\end{figure}
...
both systems the collapse hazard rises along the trajectory. The model-free
witness is the Nelson--Aalen cumulative hazard of Fig.~\ref{fig:crosssystem}c
--- the nonparametric estimator $\hat H(t) = \sum_{t_i \le t} d_i / n_i$,
accumulating deaths $d_i$ over the $n_i$ runs still at risk at each event time
$t_i$, whose local slope is the death rate itself, so a memoryless process
traces a straight line and an aging one a convex curve. Both systems' curves
are convex: the death rate after the median age exceeds the rate before it by
factors of $3.5$--$5.1$ across the mean-field sizes and $1.4$--$2.2$ across the
$\beta = 0.130$ ring cells --- though the two
...
(\texttt{fig10\_ring.py}), the cross-system comparison
(\texttt{paper\_figures/fig12\_crosssystem.py}, hazard-level checks in
\texttt{tools/unify\_checks.py}), and the measurements added in revision:
...
\texttt{tools/manifold-probe/ws\_decomposition.mjs}), the existence-boundary
measurement (\texttt{paper\_figures/fig\_betac.py}, data from
\texttt{anneal-hazard/analysis/run\_betac.py}), the depth-onset reanalysis
(\texttt{paper\_figures/fig\_depth\_aging.py}, data from
\texttt{tools/reduced-ode/depth\_aging\_fit.py}), and the ring
graze/absorption labeling measurement (\texttt{paper\_figures/fig\_xlabel.py},
data from \texttt{tools/xlabel/run\_xlabel.py}). The ring
