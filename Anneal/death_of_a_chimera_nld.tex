...
%% Nonlinear Dynamics (Springer). Venue switch only: science, results, figures,
%% and reference content are preserved byte-for-byte from the REVTeX source
%% (death_of_a_chimera_aip.tex). Reference style: sn-mathphys (Math & Physical
%% Sciences, Numbered). Swap to [pdflatex,sn-basic,Numbered] if the journal's
%% submission page specifies the Basic style instead; both .bst files are shipped.
...
\abstract{We report a long-duration empirical study of chimera collapse in the two-population
...
\keywords{chimera states, coupled phase oscillators, Kuramoto--Sakaguchi model, finite-size scaling, transient dynamics, Weibull aging hazard}
