...
degradation and a recovered bi-exponential origin.
...
precision. This strengthens the claim within its scope -- it now holds even for a
...
co-primary axes: (A) the analytic perfusion log-slope (the gamma mean, defined
without reference to a bi-exponential fit, so the test is non-circular) and (B) the
best-fit bi-exponential $\Dstar$. Calibration and recalibration use the OSIPI
...
gap persists on both co-primary surrogates (gamma mean A, bi-exp-fit B), while naive
...
gap. Three independent stresses corroborate that this split is information, not an
...
itself a hidden channel the monitor cannot see (Sec.~\ref{sec:altmodel}); and the
off-model validity envelope degrades gracefully with a recovered bi-exponential
origin (Sec.~\ref{sec:envelope}).
...
ranges and Rician noise; real tissue is more heterogeneous. The off-model behavior
is now characterized rather than probed at a single point: the high-$\Dstar$ wall
replicates under a non-bi-exponential velocity-dispersion generator
(Sec.~\ref{sec:altmodel}) and we map a validity envelope across three deviation
families with a monitored, graceful degradation and a recovered bi-exponential
origin (Sec.~\ref{sec:envelope}). These remain schematic deviations spanning the
neighbourhood of the bi-exponential model, not a validated tissue model, and the
in-vivo unverifiability above is unchanged by them. The
